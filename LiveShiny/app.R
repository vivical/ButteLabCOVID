...
parameterTableDat <- tibble(`Date Updated` = as.Date(c("2021-01-15","2020-12-15","2020-11-16","2020-10-15","2020-09-15","2020-08-15","2020-06-10")), `Hospitalization Rate` = c(0.0328,0.0443,0.0502,0.0509,0.0558,0.0826,0.1270), `ICU Rate` = c(0.1978,0.2013,0.2185,0.2607,0.22908,0.2964,0.4000),
                            `Parameter Estimation Range Start` = as.Date(c("2020-11-21","2020-10-24","2020-09-26","2020-08-22","2020-07-25","2020-06-28","2020-03-07")), `Parameter Estimation Range End` = as.Date(c("2021-01-02","2020-12-05","2020-11-07","2020-10-03","2020-09-04","2020-08-08","2020-05-30")))
# 1-14-2021
...
server = function(input, output, session) {
  ...
  output$ParameterTable <- DT::renderDataTable({
    datatable(parameterTableDat, filter = 'none', #remove column filter
      ...
      formatStyle(
        'Date Updated',
        target = 'row',
        backgroundColor = styleEqual(as.Date('2021-01-15'), c('yellow')) #1-14-2021
      )
  })
  ...
  output$stateLinePlot <- renderPlotly({
    ...
    output$doubling_time = renderText({ 
      if( input$uPlot == "ICUbeds" ){
        paste("Estimations assume a 3.28% hospitalization rate, 19.8% ICU rate") #1-14-2021
      }else{
